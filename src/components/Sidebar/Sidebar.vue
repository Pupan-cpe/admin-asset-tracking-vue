    ...
    <v-list dense>
      ...
      >
        <template v-for="item in this.items">
          <v-list-item
            :key="item.menu_name"
            @click="setDataBuilding(item.building_name)"
          >
            <v-list-item-action>
...
export default {
  ...
  methods: {
    setDataBuilding(name) {
      console.log("name", name);
      this.$store.commit("up", name);
      var smallData = this.dataFull.filter((val) => {
        return val.building_name == name;
      });
      this.$store.commit('upDateDataFlooor',smallData[0])
    },

    async getUsers() {
      this.axios
        .get("http://203.151.199.181:5002/admin/api/v1/building")
        .then((res) => {
          this.dataFull = res.data.data;
          alert(this.dataFull)
          console.log(this.dataFull.building_name);
        });
      // try {
